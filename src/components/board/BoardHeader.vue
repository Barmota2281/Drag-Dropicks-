<template>
  <header class="flex items-center justify-between mb-6 pb-4 border-b border-gray-200 dark:border-gray-700 shrink-0">
    <h1 class="text-gray-900 dark:text-gray-100 text-2xl font-bold tracking-tight">{{ boardTitle || 'Drag&Dropicks' }}</h1>
    
    <div class="flex items-center gap-4">
      <button @click="$emit('add-task')" class="px-4 py-2 bg-gray-900 dark:bg-gray-100 text-white dark:text-gray-900 text-sm font-medium rounded-lg hover:bg-gray-800 dark:hover:bg-gray-200 transition-colors shadow-sm hidden sm:block">
        Добавить задачу
      </button>
      ...
      <div class="relative">
        ...
        <input type="text" placeholder="Поиск задач и досок..." class="w-96 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 border border-gray-300 dark:border-gray-600 rounded-lg pl-10 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-gray-400 dark:focus:ring-gray-500 transition-colors shadow-sm">
      </div>
