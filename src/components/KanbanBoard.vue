            ...
            <template #item="{element}">
              <div class="bg-[#f7f3e8] text-[#2b1f1a] p-3 rounded-lg shadow group relative transition-all duration-200 hover:-translate-y-0.5" >
                ...
                <bubble-menu
                  ...
                  v-if="element.editor"
                  class="flex bg-white rounded shadow-lg border border-gray-200 overflow-hidden"
                >
                  <button @click="element.editor.chain().focus().toggleHeading({ level: 1 }).run()" :class="{ 'bg-gray-200': element.editor.isActive('heading', { level: 1 }) }" class="p-2 hover:bg-gray-100 text-sm font-semibold text-gray-700">H1</button>
                  <button @click="element.editor.chain().focus().toggleHeading({ level: 2 }).run()" :class="{ 'bg-gray-200': element.editor.isActive('heading', { level: 2 }) }" class="p-2 hover:bg-gray-100 text-sm font-semibold text-gray-700">H2</button>
                  <button @click="element.editor.chain().focus().toggleBold().run()" :class="{ 'bg-gray-200': element.editor.isActive('bold') }" class="p-2 hover:bg-gray-100 text-sm font-bold text-gray-700">B</button>
                  <button @click="element.editor.chain().focus().toggleItalic().run()" :class="{ 'bg-gray-200': element.editor.isActive('italic') }" class="p-2 hover:bg-gray-100 text-sm italic text-gray-700">I</button>
                  <button @click="element.editor.chain().focus().toggleHighlight().run()" :class="{ 'bg-gray-200': element.editor.isActive('highlight') }" class="p-2 hover:bg-gray-100 text-sm text-gray-700">Mark</button>
                 </bubble-menu>

                <!-- Оставляем GlobalDragHandle управлять этим, а мы добавляем свой дроп-хендл -->
                ...
                <editor-content :editor="element.editor" class="focus:outline-none min-h-[40px] prose prose-sm max-w-none pr-8 pl-1" />
